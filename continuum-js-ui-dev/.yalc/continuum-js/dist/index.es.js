...
import { first, multicast, filter, map } from 'rxjs/operators';
    ...
    EventBus.prototype.connect = function (url, identity, secret) {
        ...
        return new Promise(function (resolve, reject) {
            if (!_this.connected) {
                ...
                _this.replyToCri = EventConstants.SERVICE_DESTINATION_PREFIX + _this.encodedIdentity + ':' + UUID.UUID() + '@continuum.js.EventBus/replyHandler';
                ...
                var errorSubscription_1 = _this.stompClient.stompErrors$.subscribe(function (value) {
                    ...
                    var message = value.headers[EventConstants.ERROR_HEADER];
        ...
        if (!this.connected) {
            ...
            return new Observable(function (subscriber) {
                if (_this.requestRepliesObservable == null) {
                    _this.requestRepliesObservable = _this._observe(_this.replyToCri).pipe(multicast(new Subject()));
                ...
                var defaultMessagesSubscription = _this.requestRepliesObservable
                    .pipe(filter(function (value, index) {
                ...
                event.setHeader(EventConstants.REPLY_TO_HEADER, _this.replyToCri);
                event.setHeader(EventConstants.CORRELATION_ID_HEADER, correlationId);
                _this.send(event);
                return function () {
                    if (sendControlEvents && !serverSignaledCompletion) {
                        var controlDestination = EventConstants.SERVICE_DESTINATION_PREFIX + _this.encodedIdentity + '@' + correlationId;
...
var ServiceProxy = /** @class */ (function () {
    ...
    ServiceProxy.prototype.__invokeStream = function (sendControlEvents, methodIdentifier, args, scope, eventFactory) {
        var cri = EventConstants.SERVICE_DESTINATION_PREFIX + (scope != null ? scope + '@' : '') + this.serviceIdentifier + '/' + methodIdentifier;
        ...
        return this.eventBus.requestStream(event, sendControlEvents)
            ...
            var contentType = value.getHeader(EventConstants.CONTENT_TYPE_HEADER);
...
container.addSingleton(ServiceRegistry);
...
export { CrudServiceProxy, DataSourceUtils, Direction, Event, EventBus, EventConstants, JsonEventFactory, NullHandling, Order, Page, Pageable, SearchComparator, SearchCriteria, SearchServiceProxy, ServiceRegistry, Sort, StreamData, StreamOperation, TextEventFactory };
